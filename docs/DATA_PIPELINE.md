...
| `weather_dump.txt` | Open-Meteo API | `City|YYYY-MM-DD: Day D Mon: 19°C, 60% rain chance, showers` — one line per city **per game-day** (the game's local date, resolved from its kick-off), plus a dateless `City:` legacy/fallback line per city. `apply_weather` matches city + kick-off date first, city alone second. A city whose fetch fails keeps its committed line rather than going blank. No API key. |
| `injuries_dump.html` | Zero Tackle injuries & suspensions | `Team: Player (Reason) — back Round N; ...` (up to 6 per club). Return of `TBC`/unknown is left as no "— back" suffix. Kept if ≥ 6 clubs parse. |
...
- Weather cities come from nrl.com's real `venueCity`, **not** the home club's usual city
  — otherwise a regional game (Panthers v Raiders at Glen Willow Oval, Mudgee) gets the
  wrong forecast, or none at all.
...
- **Full rebuild** (default): parse ladder + home/away splits + draw, apply optional
  `--odds`/`--injuries`/`--weather` dumps, validate 17 teams, write `nrl_data.js`.
- **`--merge`** (daily reactive refresh): load the existing `nrl_data.js` and update
  ONLY `fixture.odds`, `team.news`, `fixture.weather`, and the `newsUpdated` stamp —
...
scores and **appends** them to `nrl_learned.js.results` (deduped on round+home+away;
...
Odds evolve into an `{open, close}` shape for CLV: on a full rebuild both are set to
the freshly parsed price; on `--merge`, `open` is set once (first sighting) and never
overwritten, `close` always updates. Legacy flat `{home,away}` is still accepted.
...
`logisticScale ∈ {5,6,7,8,9,10,12}` (points-space), `oddsWeight ∈ {0.0..1.0}`.
...
Fixture = {
  ...
  odds: { open:{home,away}, close:{home,away} }   // or legacy {home,away}, or null
       ,
  weather: "…, 60% rain chance, showers" | null,
  h2h: null
}
...
Change = {
  ...
  team: "SYD" | null,
  cat,                          // in | out | injury | line | weather | time | venue | other
...
Result = { round:int, home, away, hs:int, as:int }
```
...
> sorts by **severity first** — sorting by timestamp first let a spine player being ruled
> out get evicted within half an hour by a pile of 1°C weather wobbles.
...
```
{
  ...
  results: [ {round, home, away, hs, as}, ... ]   // append-only match log
}
```
...
python3 parse_nrl.py --ladder ladder_dump.html --draw draw_dump.html \
  ...
  --injuries injuries_dump.html --weather weather_dump.txt --results results_dump.txt \
  --out nrl_data.js --season 2026 --source zerotackle.com

# Daily reactive refresh (odds/news/weather only)
python3 parse_nrl.py --merge --in nrl_data.js \
  --odds odds_dump.txt --injuries injuries_dump.html --weather weather_dump.txt
